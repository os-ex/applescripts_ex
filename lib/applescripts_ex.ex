defmodule ApplescriptsEx do
  ...
  # @scripts %{
  #   send_text_group: {"SendText.scpt", to: [:tel, :email], text: []},
  #   send_text_direct: {"SendTextSingleBuddy.scpt", to: [:chat_id], text: []},
  #   send_attachment_group: {"SendImage.scpt", to: [:tel, :email], attachment: [:file]},
  #   send_attachment_direct: {"SendImageSingleBuddy.scpt", to: [:chat_id], attachment: [:file]}
  # }

  # for {key, script_relpath, validations} <- @scripts,
  # [{field_1, validations_1}, {field_2, validations_2}] = validations do
  #   def args_for(map, unquote(key)) do
  #     with :ok <- map |> Map.get(unquote(field_1) |> Validations.validate_one_of( unquote(validations_1)) ,
  #       :ok <- map |> Map.get(unquote(field_2) |> Validations.validate_one_of( unquote(validations_2)) do
